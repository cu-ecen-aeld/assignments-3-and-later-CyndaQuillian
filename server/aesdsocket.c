 ...
 * aesdsocket.c
 *
 * A stream socket server on port 9000 that:
 *  - Accepts connections one at a time (loops forever)
 *  - Receives data and appends complete packets (newline-terminated) to
 *    /var/tmp/aesdsocketdata
 *  - Sends the full file contents back after each complete packet
 *  - Logs to syslog
...
#define RECV_CHUNK      4096   /* bytes read per recv() call  */
#define SEND_CHUNK      4096   /* bytes written per send() call */
...
/* ------------------------------------------------------------------ */
/* Daemonise AFTER bind so bind errors are visible                      */
/* ------------------------------------------------------------------ */
...
{
    /* ---- Receive until newline (or EOF), accumulate into heap buf ---- */
    ...
    while (!have_newline && !g_exit_requested) {
        ...
        if (nr == 0) {
            /* EOF — process whatever we received */
            break;
        }
        buf_used += (size_t)nr;
    ...
    mkdir("/var/tmp", 0755);   /* ignore error if already exists */

    /*
     * Open for read+write+append so we can write at the end, then
     * lseek back to 0 to send the full file — one fd, no race.
     */
    int data_fd = open(DATAFILE, O_RDWR | O_CREAT | O_APPEND, 0644);
    if (data_fd < 0) {
    ...
    /* Seek back to start of file to send full contents */
    if (lseek(data_fd, 0, SEEK_SET) < 0) {
        syslog(LOG_ERR, "lseek " DATAFILE ": %s", strerror(errno));
        close(data_fd);
        return -1;
    }
    ...
    while ((rd = read(data_fd, sendbuf, sizeof(sendbuf))) > 0) {
        ssize_t sent = 0;
        while (sent < rd) {
            ssize_t ns = send(client_fd, sendbuf + sent,
                              (size_t)(rd - sent), MSG_NOSIGNAL);
            if (ns < 0) {
                if (errno == EINTR) continue;
...
int main(int argc, char *argv[])
{
    ...
    /* ---- Signal handlers (no SA_RESTART so accept() is interruptible) ---- */
    struct sigaction sa;
    ...
    /* Ignore SIGPIPE so a dropped client doesn't kill the daemon */
    ...
    /* ---- Daemonise AFTER successful bind ---- */
    if (daemon_mode) {
        if (daemonise() < 0) {
            close(server_fd);
            closelog();
        ...
        /* Re-open syslog in the child after fork */
        closelog();
        openlog("aesdsocket", LOG_PID | LOG_CONS, LOG_USER);
    }
